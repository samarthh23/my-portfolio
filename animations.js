...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (typeof anime === "undefined") {
    console.error(
      "Anime.js is not loaded - content will display without animations"
    );
    // Make sure content is visible even without animations
    document
      .querySelectorAll(
        ".hero-content, .hero-title .title-line, .hero-subtitle, .hero-description, .hero-buttons, .hero-social"
      )
      .forEach((el) => {
        el.style.opacity = "1";
      });
  ...
  function animateProjects() {
    ...
    projectCards.forEach((card, index) => {
      ...
      anime({
        ...
        translateY: [15, 0],
        duration: 400,
        delay: anime.stagger(30, { start: 400 + index * 150 }),
        easing: "easeOutQuad",
      });
    });
  }
